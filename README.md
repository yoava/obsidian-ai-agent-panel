...
- **CLI profiles**: keep several ways to launch the CLI (e.g. a Windows install and a WSL one) as named profiles with a default, and pick one per chat from the row's **Profile** control. Both are invisible while you have just one profile - which is the normal case.
...
- **Per-conversation context meter** in the composer: how full the context window is and what the chat has cost so far, with a hover breakdown of input/output/cache tokens by category.
...
Have Claude Code installed on both sides? **Add CLI profile** in settings keeps a named launch configuration for each (e.g. *Windows* and *WSL*), with one as the default; every chat can then pick its CLI from the **Profile** control under the composer. With a single profile none of this UI appears.
